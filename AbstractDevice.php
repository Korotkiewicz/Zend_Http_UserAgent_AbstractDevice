...
//                $compatibleOrIe = ($result['compatibility_flag'] == 'compatible' || strpos($result['comment']['full'], "MSIE") !== false);
            if(strpos($result['comment']['full'], "Trident") !== false) {
                unset($result['browser_token']);
                $compatibleOrIe = true;
            } elseif(isset($result['others']) && strpos($result['others']['full'], "Edge") !== false) {
                $compatibleOrIe = true;//like IE
                $result['browser_token'] = $result['others']['detail'][3][0];//use proper token
                unset($result['others']);//prevent detect safari or chrome
            } else {
                $compatibleOrIe = ($result['compatibility_flag'] == 'compatible' || strpos($result['comment']['full'], "MSIE") !== false);
            }
            /* end Bolebor mod */
